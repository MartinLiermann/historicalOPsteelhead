...
```{r fit to historical data, warning=FALSE}
# load all of the the CPUE data (historical and current)
allDat <- read.csv(file=paste(workDir,"data/cpueData.csv",sep="/"))

## Fit the beta density function to the historical CPUE data
histDat <- allDat[allDat$period=="Historic",c("population","julian","CPUE","date")]
...
```{r average current CPUE data and combine with historical CPUE data, warning=FALSE, message=FALSE}
## Fit the beta density function to CURRENT CPUE data
## because we have multiple years of current data we first calculated a single 
## composite season of weekly CPUE data using the years 1997-2010
## this was done using an anova on the log scale with main effects for year and week 

# Calculate the weekly CPUE values for an average year using anova
yrRange <- list(Quillayute=1997:2010, Hoh=1997:2010, Queets=1997:2010, Quinalt=1997:2010)
CPUElist <- list()
for(pop in currPopNames){ # for the populations with current CPUE data
  for(HW in c("Wild","Hatchery")){
    tmpDat <- allDat[allDat$period=="Current" &
                     allDat$population==pop &
                     allDat$HW==HW,]
    tmpAvg <- calcCPUEavgl(tmpDat,yrRange[[pop]])
    tmpAvg$population <- pop
    tmpAvg$HW <- HW
    tmpAvg$period <- "Current"
    CPUElist <- c(CPUElist,list(tmpAvg))
  }
}
CPUEdat <- Reduce(rbind,CPUElist)
CPUEdat$periodSt <- "1997-2010"

commonCols <- c("population","period","HW","date","julian","CPUE","E")
CPUEdat <- rbind(CPUEdat[,commonCols],allDat[allDat$period=="Historic",commonCols])

# Now fit the beta density function to the CURRENT CPUE data
...
We compare timing for the current and historical wild runs and for the current hatchery and wild runs. We use three different metrics, 1) the Julian date at which 25% of the run has past (**q25**), 2) the days elapsed between when 25% and 75% of the run passes (inter quartile range, **IQR**), and 3) the % of the run that has passed by January 1st (**pJan1**). The comparisons are made by subtracting the historical from current values. For example, $$IQR_{diff} = IQR_{current} - IQR_{historical}$$
...
## 1923 Queets River cannery records 
...
## 1948-1960 commercial and recreational catch expansion
...
allDat <- read.csv(file=paste(workDir,"data/cpueData.csv",sep="/"),stringsAsFactors=FALSE)
tcCatch <- allDat %>% filter(period=="Current",HW=="Wild",year>1989) %>% droplevels()
...
declineTab <- runDat %>% 
  ...
  left_join(estTab,by="population") %>%
  mutate(Decline = (Estimate-run)/Estimate, Current=run, Historic=Estimate) %>%
  select(Population=population, Current, Historic, Decline) %>%
  ungroup()
knitr::kable(declineTab[c(3,1,2,4),],digits=c(NA,0,0,2),format.args = list(big.mark = ",",
  scientific = FALSE), caption="Table S9. The decline in run size from the estimated historical to the current. We define decline as (Historic - Current)/Historic. For current we use the average run size from 1995 to 2017. For the historical we use the average estimated run size from 1948 to 1960, based on the historic catch expansion method.")
```

## CPUE-based historical estimates (~1955-1963)
...
qTab$RS.1 <- qTab$cpueTotHist/qTab$q
qTab$RS.1.5 <- 1.5*qTab$cpueTotHist/qTab$q
qTab$RS.2 <- 2*qTab$cpueTotHist/qTab$q

qTab$RSav.1 <- qTab$cpueTotHist/qAvg
qTab$RSav.1.5 <- 1.5*qTab$cpueTotHist/qAvg
qTab$RSav.2 <- 2*qTab$cpueTotHist/qAvg
qTab$RSav.3 <- 3*qTab$cpueTotHist/qAvg

write.csv(qTab,paste(workDir,"tables/cpueBasedEstimates.csv",sep="/"),row.names=FALSE)
```
...
```{r}
knitr::kable(qTab,caption="Table S10. CPUE based estimates of historical run size. RS.1 is historical runsize assuming efficiencies are the same and RS.1.5 and RS.2 are historical run sizes assuming that efficiency is 1.5 and 2 times higher in the current period. RSav are the estimated historical run sizes based on the average of the Quillayute and Queets efficiencies.", digits=c(NA,0,0,0,3,3,0,0,0,0,0,0,0),format.args = list(big.mark = ",",
  scientific = FALSE))
```
...
knitr::kable(datOP[c(2,4,1,3),],digits=c(NA,0,0,0,0),align=c("l","c","c","c","c"),caption="Table S11. The predicted historical population size in the four basins along with 95% prediction intervals.",format.args = list(big.mark = ",",
...
```{r warning=FALSE, message=FALSE, fig.cap="Figure S3. A side by side comparison of the different historical estimates include the Queets cannery data, the catch expansion, the CPUE based method, the river km relationship. The horizontal lines are the average run size estimates from 1995 to 2017." }
...
ggplot(cTab) +
  ...
  geom_segment(aes(x=0.75,xend=4.25,y=run/ff,yend=run/ff),lwd=1) +
...
```{r}
knitr::kable(cTab[c(3,1,2,4),],digits=0,caption="Table S12. The different historical estimates. The suffixes Est,Lwr and Upr correspond to the estimate, and lower and upper bounds. The prefix ce, cpue, rkm and can correspond to the catch expansion estimate, the CPUE based estimate, the river km based estimate and the cannery based estimate. The average run size between 1995 and 2017 is in the run column.",format.args = list(big.mark = ",",
  scientific = FALSE),row.names=FALSE)
```

# Appendix A: Sensitivity to changes in efficiency

Here we examine the sensitivity of our run timing results to the assumption of constant efficiency. By using weekly CPUE values across the season to estimate run timing, we are assuming that efficiency remains constant through time. We know that this is not true, but assume any differences do not have a large effect on our conclusions about run timing differences. Things that may affect efficiency include in river conditions due to weather (e.g. flow, turbidity, debris, etc...). For example, efficiency may be lower during periods of high flow. Fish behavior may also change across months. Fish, may be more or less grouped in different periods and may respond to river conditions like flow creating an interaction between effort and run timing, further complicating the interpretation of CPUE data. While these problems pose real issues with the interpretation of CPUE data, we believe that this approach is better than relying an raw catch data. 
...
Here's a plot of the historic(blue) vs current(green) timing using the unadjusted data.
...
Here's a plot of the historic(blue) vs current(green) timing using the adjusted data.
...
For both the adjusted and unadjusted data the historic timing is shifted to the left. The magnitude of the shift looks about the same but there are some differences in the general shapes of the curves. So, while the timing distributions can be sensitive to the addition of time-varying sensitivity, it does not change the qualitative comparison between the two time periods. If the time-varying efficiency is substantially different for the two periods, this could create problems. One might assume that the historic fisheries were more sensitive to flow (in terms of efficiency). If that were the case, then the difference might be even more pronounced since the historic timing would be shifted to the left more than the current timing.
